...
SELECT
    ...
    round(avg((total_cost - rub_monthly_fee)::numeric), 2) as overcost
FROM users_costs
    JOIN (select user_id, churn_date
    FROM telecom.users) as u using(user_id)
WHERE churn_date is null and total_cost > rub_monthly_fee
GROUP BY tariff
